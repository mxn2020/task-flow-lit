...
export class TeamDashboardPage extends LitElement {
  ...
  @state() private error = '';
  private loadInProgress = false; // Track if a load operation is in progress

  async connectedCallback() {
    super.connectedCallback();
    // Wait for the correct account to be available
    await this.waitForCorrectAccount();
    this.loadDashboardData();
  }

  updated(changedProperties: Map<string, any>) {
    // If context changed, check if it's actually different before reloading data
    if (changedProperties.has('context')) {
      console.log('[TeamDashboard] Context updated, checking if reload needed');
      
      const oldContext = changedProperties.get('context');
      const newContext = this.context;
      
      // Only reload if the team slug actually changed
      ...
      if (oldTeamSlug !== newTeamSlug) {
        ...
        console.log('[TeamDashboard] Team slug unchanged, skipping reload:', { teamSlug: newTeamSlug });
      }
    }
  }
  ...
  private async loadDashboardData(): Promise<void> {
    ...
    console.log('[TeamDashboard] Current account:', this.stateController.state.currentAccount);
    
    if (!accountId) {
      console.log('[TeamDashboard] No account ID found, waiting for account to be set...');
      this.loading = false;
    ...
    // Check if the current account matches the expected team slug
    const currentAccount = this.stateController.state.currentAccount;
    if (teamSlug && currentAccount && 
        currentAccount.slug !== teamSlug && currentAccount.id !== teamSlug) {
      console.log('[TeamDashboard] Account mismatch, waiting for correct account...');
      this.loading = true;
      this.loadInProgress = false;
      this.error = '';
      return; // Let the app-root handle the account switching
    }
    ...
    try {
      ...
      this.loading = true;
      this.error = '';
      console.log('[TeamDashboard] Starting data load for account:', accountId);
      ...
      // Load scopes with error recovery
      console.log('[TeamDashboard] Loading scopes...');
      const { data: scopes, error: scopesError } = await supabase.getScopes(accountId);
      console.log('[TeamDashboard] Scopes response:', { scopes, scopesError });
      ...
      if (scopesError) {
        ...
        if (scopesError.message?.includes('JWT') || 
          ...
          console.log('[TeamDashboard] Authentication error detected, refreshing session...');
          const recovered = await this.stateController.recoverSession();
          if (recovered) {
            clearTimeout(timeoutId);
            this.loadInProgress = false; // Reset flag before retry
            return this.loadDashboardData(); // Retry with recovered session
          }
        }
        throw scopesError;
      }
      ...
      // Load recent items with error recovery
      console.log('[TeamDashboard] Loading scope items...');
      const { data: items, error: itemsError } = await supabase.getScopeItems(accountId);
      console.log('[TeamDashboard] Items response:', { items, itemsError });
  ...
  // Method to force refresh all data (can be called from browser console or error handlers)
  async forceRefresh(): Promise<void> {
    console.log('[TeamDashboard] Force refresh requested');
    this.loadInProgress = false; // Reset flag to allow force refresh
    this.loading = true;
    this.error = '';
    
    try {
      // First refresh the state controller data
  ...
  render() {
    const currentAccount = this.stateController.state.currentAccount;
    console.log('[TeamDashboard] Render state:', {
    ...
    if (this.loading) {
      console.log('[TeamDashboard] Rendering loading state');
      return html`
    ...
    if (this.error) {
      console.log('[TeamDashboard] Rendering error state:', this.error);
      return html`
        <div class="page-layout">
          <app-sidebar 
            .stateController=${this.stateController}
    ...
    console.log('[TeamDashboard] Rendering main content');
    return html`
      <div class="page-layout">
        <app-sidebar 
          .stateController=${this.stateController}
